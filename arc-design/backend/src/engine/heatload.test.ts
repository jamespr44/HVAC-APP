import { calculateZoneLoads, checkZoning, ZoneInputs, ClimateInputs } from './heatload';
...
describe('Heat Load Calculation Engine (HAP-based)', () => {
  ...
  it('calculates OA using green star rate of 11.25 L/s/person', () => {
    const greenZone = { ...baseZone, oaMethod: 'green' as const };
    const result = calculateZoneLoads(greenZone, climate);
  ...
  it('uses max(pointLoad, W/m² × area) for equipment', () => {
    ...
    const result = calculateZoneLoads(zoneWithPoint, climate);
  ...
  it('calculates infiltration load correctly', () => {
    ...
    const result = calculateZoneLoads(zoneWithInfil, climate);
  ...
  it('calculates heating SA-T with reheat offset for non-HWC zones', () => {
    ...
    const result = calculateZoneLoads(nonHwc, climate);
  ...
  it('handles partition facade correctly', () => {
    ...
    const result = calculateZoneLoads(partitionZone, climate);
  ...
  it('handles internal zone with no facade loads', () => {
    ...
    const result = calculateZoneLoads(internalZone, climate);
    expect(result.glassSolarW).toBe(0);
  ...
  it('respects ACH-based supply when higher than load-based', () => {
    ...
    const result = calculateZoneLoads(labZone, climate);
...
describe('Zoning Check', () => {
  it('identifies good zoning when temp range < 2°C', () => {
    ...
    const result = checkZoning(zones, 12);
