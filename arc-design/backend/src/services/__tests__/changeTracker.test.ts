import { diffObjects, generateChangeSummary } from '../changeTracker';
...
describe('Change Tracker', () => {
  ...
  describe('generateChangeSummary', () => {
    ...
    it('handles entity name in summary', () => {
      ...
      const summary = generateChangeSummary(changes, 'zone');
  ...
  describe('Equipment Change Tracking', () => {
    it('tracks equipment capacity change', () => {
      ...
      const diff = diffObjects(oldEquip, newEquip);

      expect(diff.totalCapacityW).toEqual({ from: 5000, to: 6000 });

      const summary = generateChangeSummary(diff, 'equipment');
  ...
  describe('Zone Change Tracking', () => {
    it('tracks area change', () => {
      ...
      const diff = diffObjects(oldZone, newZone);

      expect(diff.areaM2).toEqual({ from: 100, to: 120 });

      const summary = generateChangeSummary(diff, 'zone');
