...
export interface ZoneValidationInput {
  ...
  windowSHGC?: number | string;
  windowShadingFactor?: number | string;
  infiltrationACH?: number | string;
  occupants?: number | string;
  equipmentWm2?: number | string;
  lightingWm2?: number | string;
  tempCoolingC?: number | string;
  tempHeatingC?: number | string;
  humidityMinPct?: number | string;
  humidityMaxPct?: number | string;
}

export function validateZoneInput(input: ZoneValidationInput): ValidationError[] {
  const errors: ValidationError[] = [];

  // ── Geometry Validation ──

  // Area
  const area = input.areaM2 ? parseFloat(input.areaM2 as any) : undefined;
  if (area !== undefined) {
    if (area <= 0) {
  ...
  // Height
  const height = input.heightM ? parseFloat(input.heightM as any) : undefined;
  if (height !== undefined) {
    if (height < 1) {
  ...
  const wallUValue = input.wallUValue ? parseFloat(input.wallUValue as any) : undefined;
  ...
  // Window U-value
  const windowUValue = input.windowUValue ? parseFloat(input.windowUValue as any) : undefined;
  ...
  // Window SHGC (0–1)
  const shgc = input.windowSHGC ? parseFloat(input.windowSHGC as any) : undefined;
  if (shgc !== undefined) {
    if (shgc < 0 || shgc > 1) {
      errors.push({
        field: 'windowSHGC',
  ...
  const shadingFactor = input.windowShadingFactor ? parseFloat(input.windowShadingFactor as any) : undefined;
  ...
  const windowArea = input.windowAreaM2 ? parseFloat(input.windowAreaM2 as any) : undefined;
  ...
  const ach = input.infiltrationACH ? parseFloat(input.infiltrationACH as any) : undefined;
  if (ach !== undefined) {
    if (ach < 0.01) {
      errors.push({
  ...
  const occupants = input.occupants ? parseInt(input.occupants as any) : undefined;
  ...
  // Equipment load
  const equipmentWm2 = input.equipmentWm2 ? parseFloat(input.equipmentWm2 as any) : undefined;
  ...
  // Lighting load
  const lightingWm2 = input.lightingWm2 ? parseFloat(input.lightingWm2 as any) : undefined;
  ...
  const coolTemp = input.tempCoolingC ? parseFloat(input.tempCoolingC as any) : undefined;
  const heatTemp = input.tempHeatingC ? parseFloat(input.tempHeatingC as any) : undefined;
  const humidityMin = input.humidityMinPct ? parseFloat(input.humidityMinPct as any) : undefined;
  const humidityMax = input.humidityMaxPct ? parseFloat(input.humidityMaxPct as any) : undefined;
...
export function validateCalculationResults(results: {
  ...
  if (
    results.transmissionW !== undefined &&
