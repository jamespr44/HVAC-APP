import { validateEquipmentInput, validateEquipmentAgainstZone } from '../equipmentValidator';
...
describe('Equipment Input Validation', () => {
  describe('Equipment type validation', () => {
    it('accepts valid equipment types', () => {
      ...
      validTypes.forEach(type => {
        const errors = validateEquipmentInput({ equipmentType: type, name: 'Test' });
        expect(errors.filter(e => e.field === 'equipmentType')).toHaveLength(0);
      });
    });

    it('rejects invalid equipment type', () => {
      const errors = validateEquipmentInput({ equipmentType: 'heater', name: 'Test' });
    ...
    it('rejects missing equipment type', () => {
      const errors = validateEquipmentInput({ name: 'Test' });
      expect(errors).toContainEqual(expect.objectContaining({
        field: 'equipmentType'
      }));
    });
  ...
  describe('Name validation', () => {
    it('rejects empty name', () => {
      const errors = validateEquipmentInput({ equipmentType: 'ahu', name: '' });
      expect(errors).toContainEqual(expect.objectContaining({
        field: 'name'
      }));
    });
    ...
    it('rejects missing name', () => {
      const errors = validateEquipmentInput({ equipmentType: 'ahu' });
      expect(errors).toContainEqual(expect.objectContaining({
        field: 'name'
      }));
    });
    ...
    it('accepts valid name', () => {
      const errors = validateEquipmentInput({ equipmentType: 'ahu', name: 'Main AHU' });
  ...
  describe('Air flow validation', () => {
    it('requires air flow for AHU', () => {
      const errors = validateEquipmentInput({
        equipmentType: 'ahu',
        name: 'AHU-01'
      });
      expect(errors).toContainEqual(expect.objectContaining({
...
describe('Equipment vs Zone Validation', () => {
  it('returns error when equipment undersized (< 0.95 × zone load)', () => {
    const result = validateEquipmentAgainstZone(4000, 5000); // 4000 < 0.95*5000
    expect(result.error).toBeDefined();
    expect(result.error).toContain('undersized');
  });

  it('returns warning when equipment oversized (> 2 × zone load)', () => {
    const result = validateEquipmentAgainstZone(12000, 5000); // 12000 > 2*5000
    expect(result.warning).toBeDefined();
    expect(result.warning).toContain('oversized');
  });

  it('returns no warning when equipment is properly sized', () => {
    const result = validateEquipmentAgainstZone(5500, 5000); // 5500 is between 0.95*5000 and 2*5000
    expect(result.error).toBeUndefined();
    expect(result.warning).toBeUndefined();
  });

  it('returns warning when zone load is not calculated', () => {
    const result = validateEquipmentAgainstZone(5000, 0);
    expect(result.warning).toBeDefined();
  });
});
